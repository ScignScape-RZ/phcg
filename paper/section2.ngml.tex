...
\p{In a context-senitive grammar, not every rule-match will 
directly modify an output graph; instead, the result of a 
match might be to alter the parsing context.  For example, 
%?Figure~\ref{fig:figStringContext} \vsftcl{h}{figStringContext} 
...
can look like at \TwoOverlay{} in Figure~\ref{fig:figStringContext}: 
%?lstinline{flags_all_(parse_context ,inside_string_literal), run_context}.
}  
...
parsed.  Figure~\ref{fig:figAddToken} \vsftcl{h}{figAddToken} samples code 
logically following Figure~\ref{fig:figStringContext} wherein 
...
\lstinline{,fnd ::: Fn_Doc*;} \mdash{} is usually the introduction of a 
...
\p{Once this provisional anaysis has run its course, the system then 
...
steps.  Figure~\ref{fig:figIR} \vsftcl{h}{figIR} samples a file in the 
...
Figure~\ref{fig:PGBIR} \vsftcl{h}{figPGBIR} shows some 
...
\p{The intermediate language designed via thexe techniques is effective 
...
\IR{} via a method interface.  Figure~\ref{fig:figPGB} \vsftcl{h}{figPGB} 
