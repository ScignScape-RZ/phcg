...
\p{I have just left a lot of loose ends: in particular, these comments 
lack any philosophical or technical definition of \q{representation}.  Well, there 
...
Moreover, aside from these relatively formal modes, 
we have a capacity to indirectly describe information via 
natural language.
}
...
Akin to Natural Language, digital data often arises from 
textual representations, via 
special \q{languages} like \XML{} or \JSON{}.  As with 
...
mathematical representations, their digital peers can be 
analyzed as instances of spaces or structures with certain 
algebraic or syntactic properties and requirements.
}
\p{It is often possible to mathematically describe the 
...
consistent with its protocols.  Also, each data 
...
a single root element or (insofar as such a structure could be 
representable) with mismatched tags, etc.)}; 
\i{traversability} (a notion of parthood, and 
by extension operations of iterating
or refocusing among parts); and, let's say, \i{atomicity} 
(a notion of unitary parts that have a representation or interpretation 
as integral wholes).  These criteria ensure that digitally 
...
\p{Meanwhile, atomic units can be textually 
...
be copied and distributed.  Finally, digital data-structures 
representations can be marshaled into different binary forms 
\mdash{} encoded in byte- and bit-patterns \mdash{} for both 
database storage and live-memory  
application access.  Ultimately, data structures map to 
\q{typed values} manipulated by application-level runtimes. 
}
\p{I take the time to lay out these basic principles because I'd like  
...
protocols for encoding and exchanging data between network endpoints;  
...
while application \i{developers} focus on how to 
properly encode data structures for binary runtimes.  Such varying 
perspectives all influence the theory and technology 
behind digital data representation \mdash{} a successful 
representational paradigm needs to adapt to the operational 
requirements of engineers in each of these disciplines.
}
...
a scientific-software application (maybe years later).   
...
\p{So these are all factors contributing to how 
digital representation strategies 
become popular with scientists and engineers.   
Over the decades, many strategies and representational 
...
to structurally looser variations on row/{\allowbreak}column 
...
architecture codified by \i{Web Ontologies}, although 
other graph-representation models (such as 
Conceptual Graph Semantics) were also candidates for 
potential technological \q{entrenchment} before 
Semantic Web formats like \RDF{} and \OWL{} 
became standardized.\footnote{Why \RDF{}/\OWL{} and not, say, Conceptual 
Graphs, hybrid Object-Database/Graph-Database models, 
became canonized in the Semantic Web, is 
an interesting question but perhaps of mostly historical 
interest insofar as Hypergraphs can unify each of these paradigms.
...
contexts prerequisite to a decentralized digital ecosystem 
\mdash{} databases, networking, application implementation, visual/interactive 
software design, and semantic expressiveness \mdash{} more effectively 
than other paradigms, like regular graphs or \SQL{}-style tables.
}
\p{There has also been a recent uptick in 
research on hypergraphs in such fields 
as Category Theory, Information Management, Artificial Intelligence, 
and Natural Language Processing.  Compared to 
the Semantic Web, however, one observes a noticeable lack 
...
\p{This situation may be explained in part by subtle differences in 
...
\p{In the Semantic Web case, structures such as 
\RDF{} \q{bags} and \q{collections} introduce a kind of 
hierarchical organization \mdash{} qualifying as 
...
the higher-level structure is indirectly signaled by 
\lstinlinestyle{rdf:type} assertions on individual nodes, which 
\i{imply} without explicitly \i{constructing} a larger graph 
pattern.  Hypergraphs become \i{de facto} embedded within ordinary graphs, 
...
ordinary graphs (assuming a suitable labeling convention), 
...
\p{As I will outline in the first full section, 
there are at least some half-dozen 
different structures that may certainly be described as hypergraphs, generalizing 
various underlying graph models (even restricting attention to 
labeled and directed graphs).  Within this space of possibilities, 
...
run computer code that is in a suitable representation).
}
\p{This 
...
environments.\footnote{Two frameworks I know of along these lines are OpenCog, which 
I will touch on below, 
and LMNtal from Waseda University in Japan 
(\dblhref{https://www.ueda.info.waseda.ac.jp/lmntal/index.php}).
}
}
...
acknowledge that such descriptions are possible.  Instead, my 
...
computational procedures via hypergraph structures.  The 
...
code.  (The demo may be obtained here: 
\raisebox{2pt}{{\footnotesize 
\dblhref{https://github.com/scignscape/PGVM}}}.) 
}
...
any representative of the space of instantiations of a 
...
Such is one avenue toward the goal of a general-purpose 
specification of the space of possible hypergraph structures 
across disciplines. 
}
...
one hypergraph, from which a second is populated.  In conjunction 
with serialization and parsers, hypergraphs can then be realized 
...
Grammars, serialization and deserialization, and \IR{} thereby become
part of the formal architecture defining a hypergraph ecosystem and 
inter-graph transformations \mdash{} analogous, roughly, 
to the trio of \XML{} parsers, \mbox{\XML{} transforms/\XSLT{}}, 
and the \XML{} Document Object Model (\DOM{}).
}
\p{Continuing the \XML{} analogy, note that \XML{} is not just a serialization specification: 
the full \XML{} technology defines requirements on a series of tools 
...
requirements on the traversals which the derived structures 
\mdash{} the so-called \q{Post-Processing Infoset} \mdash{} must 
support); and for inter-document mapping 
(via \XSLT{} or \XMLFO{}).  The \DOM{}, and the 
\XML{} Infoset, is the \XML{} 
format's structural core, even more than any surface-level syntax (the grammar 
for tags, attributes, and so forth).  Analogous specifications 
would have to be developed for a generalized Hypergraph representation.
}
\p{The paper's demo code does not purport to provide a mature or 
decisive implementation of a general-purpose hypergraph ecosystem, 
but rather to suggest by \mbox{example} what components may comprise 
such a framework and how they may interoperate.  The code base includes 
preliminary and 
intermediary structures for connecting hypergraphs to 
a surrounding computational infrastructure \mdash{} parsers, 
Intermediate Representation, application-in\-tegration  
