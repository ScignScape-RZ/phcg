...
than just distinguishing incoming and outgoing dges) which share a 
source or target node.  For example, the set of incoming edges 
to the same target node may be partitioned into distinct 
...
status (\visavis{} name resolution, function visibiloty, and 
polymorphism).  This calls, under the graph model, for splitting 
incoming edges into two \q{channels}, one reprsenting regular 
