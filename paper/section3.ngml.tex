...
input \nClrSym{} that is then also input to \fFun{}.  Here \gFun{}'s 
...
with \zClrSym{} \mdash{} the latter edge conforms to the relation of 
...
\p{Some programming language accommodate these 
...
type \tyssm{} from a reference type \tyRef{}.  This operator naturally 
translates to a function \tyRefmapstoTy{} that retrieves the 
value held by a reference.  It is appropriate to analyze this 
function as a \mbox{\tyRefrightarrowTy{} morphism}.
}
...
in the called procedure's signature and implementation \mdash{} which 
...
\q{message receiver} (\tthis{}) object; so   
...
such as method-calls and exceptions \mdash{} where languages depart 
...
\p{What these two semantic regimes have in common 
...
allows method-chains to be compactly written) \mdash{} but 
...
\p{Under such interpretation, graphs modeling computer code 
...
demonstration \mdash{} which earlier I covered 
mostly from a grammar and parsing angle \mdash{} consequently 
...
(which I call \PGVMIR{}, for \q{PhaonGraph Virtual Machine}) 
...
\p{As described, \PGVMIR{} plays a limited role insofar as it is 
generated from channel-package objects \mdash{} yielding 
\PGVMIR{} files \mdash{} and then sometime later a virtual machine 
runs the \PGVMIR{} code to recreate those objects at runtime.  
In this role \PGVMIR{} just allows the channel-package objects 
...
\p{The \PGVMIR{} format exploits the fact that every minimal virtual-machine 
operation has a similar form \mdash{} each 
...
\p{The final step for a PhaonGraph Virtual Machine is to 
...
(and also \tthis{} objects and exceptions) from 
...
code representations.  However, a \PGVM{} environment eventually 
defers to \Cpp{} functions for underlying 
functionality, which implies that packages based on exotic 
protocols must be mapped to simpler packages shaped around 
\Cpp{}: at most one \tthis{} and one return value; 
...
signatures \mdash{} at runtime, \PGVM{} can access \Cpp{} functions 
which are described ahead of time in terms of the 
channel-bundles.\footnote{Separately, \PGVM{} can also call \Qt{}-specific methods according 
to \Qt{}'s internal reflection mechanism, the Meta-Object Protocol.
}  When a \PGVM{} instance starts up, the predefined 
...
called from the same site in the \PGVM{} engine.  
