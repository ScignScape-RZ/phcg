...
peers \mdash{} a process which itself involves language, 
but language in a different register, meta-discursive 
more than semantic.  
}
\p{One way of describing this is to posit that what we 
...
that can be \q{handed off} to other isciplines, 
...
in highly formal milieus.  For xample, the digital 
...
and \q{concrete} binary-electrical codes.  Progreamming language
...
opaque.  In functional programming, for example, we can associate each type 
with an algorithm that can construct every lement 
...
each of its value \mdash{} for instance, every list 
...
prove something about my impkementation of a 
...
say, I can get a list by dereferencing a list-pointer, 
