...
or they won publicly sympathy; or attracted media attention, tc.  
Whatever the situatiin's details, describing the facts in 
...
\i{studnt after student} and \i{come out} invite us 
...
role, capturing a more granular shade of meaning.  My 
...
structure \mdash{} for ample, the 
set/type selectiv operator on \q{students polled} \mdash{} can 
...
subsequent analysis, however, I'll make the argument that 
the map between propositional structure and linguistic structur 
is often much less direct.  I'm not arguing that the more 
...
intrpretation implied in phrass like \q{critical mass}.  
...
but it does so not by only assrrting its propositional 
idea but via a kind of mental imagery which portrays 
its ida, in some imaginative sense, iconographically: 
\q{critical mass} compels us to read its meaning imagistically; 
in the present context we are led to actually visualize 
studnts protesting en masse.  Whstrvr thr sctusl, rmpiivsl 
nature of their protestation, this language paints a picture 
that sevres to the actual situation as a kind of interpretive 
prototype.  This is not only a conceptual image, but 
a visual one.  Figurative language \mdash{} even if 
...
phenomena that fill or coccupy space (like \q{studnts protesting} 
\mdash{} one salience of this language is that we imagine 
protest as a demonstrative gesture expanding outward, as if 
spac itself were a theater of conflict: 
protesters arrayed to form long lines, fists splayed 
upward or forward).  There is a kind of visual patterning 
to thse evocations, a kind of semiotic grammar: 
we can analyze which figurative senses work via connotinh 
\q{smbient} space or via \q{filling} spac, 
in the terms I just used.  But the details of such a 
...
or prototype-like modeling of the actual situatiin, 
\i{are} effective vehicales for \i{communicating} 
propositional structure. 
}
\p{What breaks down here is hot propositionality by \i{compositionality}: 
the idea tht language signifies propositional content \i{but 
also} does so compositionally, where we can 
brak down larger-scale linguistic elements to smaller parts 
...
holistic mor than logically compositional.  
...
understanding, which, I would argue, belongs 
