...
and memory addresses that \q{carry} a valu \mdash{} is 
...
\p{Type-theoretically, annotative partiality extends 
conventional applied type theory 
...
their types.  When a procedur is \i{called} these values 
...
(as formal objects) deal with potntialities 
...
the information th callee requires \mdash{} thereby 
...
values in order to produce concrete values.  
...
profile, we can analogously defined 
...
calculated \i{result}, just as th type of 
a function is different than the type of its 
...
\q{exprssion annotative partials} and 
\q{signature annotative partials}, a projection 
of head/tail duality in an abstract thory 
