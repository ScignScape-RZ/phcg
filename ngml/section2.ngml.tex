...
triggers an intrepretation shaping how the plurality is picturd and 
...
collections: 
\begin{sentenceList}\sentenceItem{New Yorkers live in one of five boroughs.}
\sentenceItem{New Yorkers reliably vote for Democratic presidential candidates.}
\sentenceItem{New Yorkers constantly complain about how long it takes to commute 
to New York City.}
\end{sentenceList}
...
\end{sentenceList}
According to truth-theoretic semantics, sentences compl addressees to believe 
...
quasimathematical phrases like \q{the majority of}).  But most sentencs 
...
\p{It is also true that the more \q{logical} versions are mor, we might say, 
...
audienc, less subject to potential confusion, and indeed such 
...
such discours comes across as self-consciously removed from 
day-to-day languag.  More important, the current examples show 
that if addressees \i{have} the requisite background knowledge, 
...
\end{sentenceList}
Plurality here is introduced not only by isolatd morphology (like \q{slices}, 
...
\p{In the last five above cases, \q{it} binds (being singular) to \q{the franchise} 
seen as a single unit, but also to the \q{core of young players}.  The 
players on a team can be figured as a unit or a multiple.  The franchise itself 
can be treated as a multiple (the various team xecutives and decision-makers), 
...
that the star \i{players} lack enough depth implies that they cannot excute 
plays during the game as effectively as with better supporting players; 
asserting that the \i{franchise} lacks depth maks the subtly different point 
that there is not enough talent over all. 
}
\p{The unifying thme across these cases is that when forming sentences we often 
have a choic of how we figure plurality, and moreover these choices can be 
...
singular or more multitudinal aspects of the aggrgat in question.  But this effect 
is not localized to th individual it/they choice; it depends on 
tracing the pronoun to its antecdent and construing 
how the antecedent refrent has both individuating and multiplicative propertis.  
Thus both individuation and plurality ar latent in phrass like 
\i{slics of} or \q{cohort of}, and this singular/plural co-conceiving 
...
interprtation to identify the propervconceptual scope of plurals.  
...
typically actually perform.  Likewise in \q{The team flew to 
New York and they played the Yankees}, only the players are referenced via 
\q{they played} but presumably many other people (trainers, coachs, staff) 
...
are reinterpreted and redirectd:
...
I disucssed similar modulation in concptual schemas rlated to plurality 
and pluralization earlier; what is distinct in ths last examples is how 
the interprtive processes for cognizing plurality are shaped by 
agreement-patterns (like \i{it} or \i{they} to a composit antecedent) as 
much as by lexical choice and morphology in isolation.
}
\p{I have accordingly outlind a theory where lexical, morphological, and 
...
and conceptualization) ultimately deliver linguistic maning.  What is 
...
levels of complxity, the substance of linguistic rsearch, on this view, 
...
\p{This higher scale is my focus in the nxt section \mdash{} seen from the 
