\spsubsectiontwoline{\q{Criteriological} and \q{Expository} Mereology}
...
smaller into something larger.  The linguistic 
template \i{$x$ is part of $y$} is usually interpreted 
in such a way that $y$ actually has some existence or aspects 
\i{not} part of $x$, but the current speakr's attention 
is focusing on contexts where $x$ and $y$ overlap.  
That's why we don't 
...
say that the propositional form of these examples 
...
involving explicit parthood.  From the angle of 
NAM set against Classical mereology, of course, 
overlap and parthood have a significant 
difference: overlap is symmetric while parthood is not.
}
\p{I'll give another example set on the lost continent 
...
be part of the Atlantis Times: perhaps its offices are in the 
Times's headquarters, and the Times funds, administrates, and legally 
controls the Consortium.  But we can also 
say that the Times is part of the Consortium, if 
the Consortium includes a portfolio of papers 
...
th original portal!  Reading \q{includes} as \q{has as part}, 
this is clearly a partonomic cycle.  Indeed the whole 
...
structures where non-anti-symmetry is a more effective 
modeling assumption than anti-symetry.  In that case 
...
I will pay special attention to cases where both 
...
but call \chpp{} the transitive closure of 
\hpp{}.  Then (*) exemplifies a case where 
$x$ \chpp{} $x$.
}
\p{Consider the Atlantis example, which posed that as 
the Consortium being an \i{administrative part} 
of the Atlantis Times, whilr the Times 
is a \q{constituant} part of the 
Consortium \mdash{} if someone listed the papers in the consortium, 
...
\p{However, there are millions of notions of parthood, and 
mereology is not very useful if we can 
...
This implies building in to mereological 
theories a sense of the conceptual structure of 
the parthood relations thereby theorized. 
}
\p{I'll start by defining \hpp{} 
as something like: \q{\i{has as proper part by all relevant 
...
\p{Someone looking from outside might prefer to say that 
...
and tangential to the department.  A specialized imprint 
...
concerning the relationship on the whole's terms. 
}
...
stray $z$ in $y$ not be 
a \i{potentially relevant} part of $x$.
}
...
stipulate that there is only one global set of 
...
\p{As I have maintained, 
things are not in most common-sense 
perspectives \i{completely part} of 
...
We use the referential relation between his \i{name} and 
...
relevant to les Bleus \mdash{} their training, formation, tactics, 
marketing, popularity, etc.  Obviously 
not every part of Mbappe is relevant to the team; he 
does not train with them every minute.  Indeed, he has a whole 
other career at Paris St.-Germain.  But aside from the 
cognitive complexity of tracing in what senses he 
...
relations is then a matter of $x$ being concptually figured as a 
...
which introduces the posibility of enumerating its 
members, which thereby become its \i{parts}.  But 
rarely are part/whole relations thereby conceived 
where the whole \q{fully surrounds} the part, absorbing it so 
...
introduced to things via reference to a containing whole, 
and usually that whole is figured as a plurality, 
collection, or type (rather than as a physical or 
...
attention can migrate from whole to part.  It is easy 
...
Bleus; then we learn about Mbappe's career, of which the 
...
networks can be understood as a mereology; 
...
or of \CENTCOMM{}, as if these were geographic territories.  
...
mereological relations.  Surely the 
...
\q{exposes} some affiliated resource $R$.  Here saying 
...
smaller than wholes \mdash{} seems readily contradicted.  
I will consider the consequences of this \q{cross-referencing} 
in the final section of this paper.
}
\p{Taking a cue from the \q{linked data} nature of 
Information Spaces, the underlying model of parthood 
in this kind of theory might involve 
some form of epistemological linkage, where access to 
...
call (borrowing a term from Cognitive Grammar) 
...
designatory relation between proper names and their 
...
Combining \wtp{} and \profriso{} yields a three-part 
...
\q{criteriological} mereology: suppose the 
...
where profiles are \i{not} in general parts of what 
they profile.  Instead, a profile is 
...
I'll call this model \q{Expository} mereology.  
The canonical idea of parthood is now that 
...
family, etc.  This might be changing 
the subject somewhat: \q{overlap-systems} 
characterized by generally complex entities that overlap 
...
with mereology emerging as we attend more 
to the filtering effects of \xhppy{} parthood, 
the relation either witnessing or effectuating 
our disposition to ignore non-$x$ parts of $y$.
}
\p{To put it differently, arguably 
...
So \q{relevance} can be like a \q{knob} 
tuning in mereological \i{or} overlap theories 
...
mereologies.  In a 
mereological system, profiles need not have their 
own parts, because any internal structure belongs not to 
the profile in itself but to the thing profiled.  
Accordingly, on Classical terms, 
we can have only one \q{wholes} layer 
and only one \q{parts} layer: there are wholes whose 
parts are profiles, and profiles refer to other 
wholes.  This two-layer architecture takes the place 
...
We could treat $s$ as a whole and model its members 
...
a lower level with profiles, and an intermediary 
level which contains most of the primary 
objects of investigation or conceptualization.  Profiles can 
target multiple objects at this mid-level by targeting a frame 
rather than a single object.\footnote{The significance 
...
complex wholes with some organizational oherence, such 
...
intgrated wholes that often function as singles, 
from fiat wholes that warrant encircling in a specific 
