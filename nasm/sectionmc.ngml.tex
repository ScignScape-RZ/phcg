...
Here (\ref{itm:related}) forefronts rhinos as a natural kind, invoking a familiar 
...
relative to the menu?  Taking departure from Formal Concept 
...
to be \q{Ontologically Complex} \mdash{} reverting here to the 
philosophical rather than computational sense of \q{ontology} 
\mdash{} which is to say that there is no single fashion in which they 
...
its staff, menu, and building.  Yet \q{This restaurant is 
vegetarian} is not a mere abbreviation for \q{This restaurant's 
menu is vegetarian}.  By analogy, if a book has a yellow cover 
or a house has yellow bricks, we can say \q{this house/book is yellow}.  
That does not imply that the internal decor of the house has much yellow, 
or that the book's paper sheets are not white.  Instead, a property of the 
visible exterior is plausibly predicated of the whole.  Such 
mereological property-attribution carries over to the examples 
of a restaurant's menu/staff/building, but the \q{mereology} in 
these cases is subtler because it involves different \q{registers}.  
Certainly the staff, building, etc., is \q{part} of a restaurant.  
But a \q{sum of parts} here is not simplistic set-theoretical union; 
...
and spatial aspects \mdash{} the latter including having a reasonably 
private area in which to eat, the option of using rest-rooms, etc.) 
\mdash{} to cross ontological \q{registers}.  
}
\p{As you may have guessed, I am skeptical about this model-theoretic 
machinery as a proper vehicle for natural-language semantics.  
To be fair, we should assess any \q{formal} semantics against 
a presumptive distinction of \i{semantics} and \i{pragmatics}, 
such that linguistic nuances which seem to violate model-theoretic 
...
multiple, slightly different but overlapping purposes.  To be 
precise, the \q{City of London} is technically a small district 
...
attached to the Bronx.  If we accept a \q{model-theoretic} paradigm 
...
Conventional usage in the two cities suggests that this question is 
...
as the \q{real} (referent of) Manhattan.  Maps on the westbound 
\$textcircled{F} train show Roosevelt Island as the first stop in 
...
Distinguishing \i{semantics} from \i{pragmatics}, we might 
argue that there is nonetheless an underlying semantic 
system here where the two locations are clearly differentiated, 
and that mixed usage for the name \q{Manhattan} reflects a 
multifaceted map from pragmatics to semantics.  In other words, 
the name \q{Manhattan} reflects a pragmatic, communicational 
speech-gesture which singles out one or another formal-semantic 
meaning depending on context; underlying the surface pragmatics 
...
make this distinction, not because it can be inferred from context, 
but because it is not relevant.  For example \mdash{} to borrow an 
example due to Mark Heller, a philosopher who has written extensively 
on the \q{theory of reference} and, not coincidentally, has had a long 
career teaching in Syracuse \mdash{} the assertion \i{Syracuse is north of 
...
another example, he points out how a sculpture may begin as a 
...
overlapping with it for much of its history.  (To vary 
the example, imagine the sculpture being repaired but using some 
other clay: now the statue continues on but as a different lump of 
clay.)  In \cite{JubienOntology} Michael Jubien counters this analysis 
...
Depending on contexts and speaker's particular cognitive orientations, 
...
\p{Jubien's property-theoretic semantics, I believe, bears some features in 
common with the functional orientation I am pursuing here.  Suppose someone 
actually asserts, or thinks to themselves about how \q{Syracuse is north of 
Manhattan}.  They may be intending to drive down from Syracuse University to 
hear a lecture at Columbia (to imagine one of many possible scenarios).  
...
Geneseo to Columbia \footnote{the university, not the country} as 
...
\p{This functional perspective on semantics is fundamentally 
different from a model-theoretic paradigm, and it casts 
doubt on the very notion of a formal semantics 
\i{separate and apart} from pragmatics.  There \i{is} no 
meaning apart from usage.  In other words, semantics 
is not a mathematical map between a lexeme 
(or any other representational artifact, like a 
computer data structure) and an empirical 
object (or place, region, event, etc.); rather, 
semantics governs the \i{functional} associations 
activated between representational media and 
empirical phenomena, insofar as both representations 
and empirical surroundings \mdash{} and conventionalized as 
well as ad-hoc connections between the two \mdash{} are 
horizons and settings of human activity.  
A model-theoretic semantics has to assume that, 
...
\p{Th consequence for conventional mereological analysis 
...
\p{I will return to these issues in a latr section.  At this point, 
