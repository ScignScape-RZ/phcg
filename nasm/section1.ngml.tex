\section{Mereology as a Cognitive Phenomenon}
\spsubsectiontwoline{Mereotopology and Phenomenological Discontinuity}
...
discontinuities in the flag as an (otherwise) 
integrated union of material and color.  
...
or celebrated aspects of a community's culture and history.  
At the same 
...
\p{Mereology in this kind of analysis can be more general than 
...
clearly see possible moves for the different pieces.
}
\p{As such, a board-game \q{square} 
...
In Peter G\"ardenfors's Conceptual Space theory, color is a canonical \q{geometrized} 
...
\p{Geospatial semantics provide good examples of both 
of these phenomena.  
...
Manhattan but not the island of Manhattan; also, the borough integrates 
...
\p{According to Olav K. Wiegand, 
\begin{quote}The unfamiliarity of an object exhibits itself 
not by complete absence of anticipations that are 
inducd by the type to which it 
belongs, but rather by the degree to which 
the typical latitude (\i{Spielraum}) of 
anticipations is determined.  Hence, the 
\i{familiar} things of everyday life 
are exactlythose things, perceived by virtue of their 
typicality, that possess a minimal amount of 
indeterminacy in their normal characteristics.  Such 
familoar objcts give rise to a certain \q{intimacy} and 
\q{automatism} in our dealings with them.  However, the 
Phenomenologist views and analyzes these phenomena as a 
highly complicated structure of 
\i{explicit} and \i{implicit} expectations and memories.
\\\vspace{.25em}
A crucial point here, however, is that the 
anticipated perceptions, to which the present one points, 
are subject to two conditions: they must be in agreement 
and conformity both with one another and with the 
present perception....  Th individual object is 
thus an \i{organized totality}, the members of which are 
subject to mutual \i{functional dependencies}.  
These dependenciescan now be clarified as having 
two aspects: (a) each individual noma has a functional 
significance for the whole (on could say tht 
it maks demands on the whole in order to fit into the 
whole) and (b) at the sam time the complete noematic system 
determines the functional significance of each of its 
parts.  Both of these make up what Gurwitch 
calls the \sq{principle of sense conformity}.  Expressed 
noematically, the principle states that the complete 
noematic system must be constituted in 
such a way that it can adopt each perceptual 
noema as a part or element of itself. 
\cite[p. 14]{WiegandSemantics}
\end{quote}  The likely implication is that 
protention and retention are not based 
solely on perceptual continuity \mdash{} imagine the 
continuous perceptual path accompanying the 
act of turning over an object held in one's hand 
\mdash{} but more significantly on our functionalized 
exprience of objects as structured wholes.
}
\p{What I have hoped to emphasize in the discussion so far, in any case, 
...
patterns permit (no less than \q{sense data}) 
the formation of attentional foci.
}
\spsubsectiontwoline{Individual Coherence and 
Internal Structuration}
\p{On this reasoning, a \i{cognitive} theory of 
mereology can originate with \i{perceptual} 
discontinuity but then generalize outward.  A part is 
experienced as such not only because it is 
visually or sensorily distinguishable from 
its surroundings, but because it has some 
functional integrity, perhaps reinforced by 
conceptual specificity: the zipper in a 
jacket, the knob on a stovetop, 
the promotion rank on a chessboard, etc.  
A thorough \q{cognitive mereology} should therefore 
both differentiate these perceptual/functional/conceptual 
foundations and also consider the structural accounts that 
unite them \mdash{} there are aspcts of parthood 
that are correspondingly envincd in each partonomic 
\q{mode}.  For example, whether we ar econsidering the 
relation perceptually, functionally, 
or conceptually, a part is necessarily 
both continuous with and detached from the whole, 
in appropriate senses.  A part, therefore, 
necessarily has some \i{individual coherence}, 
some features warramting its being experienced as 
noticably distinct from the whole on some 
attentional premise.  
}
\p{Meanwhile, a whole must in turn have individual coherence, 
for it to be other than a trivial assortment of 
other wholes.  The very idea of a \i{whole} implies a 
balance between unifying criteria and sufficient 
inner discontinuity, or \i{internal structuration}, 
to be appropriately cognized as a whole \i{with parts}.  
A putative whole with no coherent rationale 
can be deemed \q{trivial} insofar as a random assortment 
of physical objects, for example, is not itself 
a physical objects.  On the other hand, an atomic or 
undifferentiated whole is trivial in a different sense, 
given that it becomes seemingly a partless a whole, a 
whole removed in its own part from mereological considerations.  
}
\p{In this latter kind of case a putative whole may 
have strong criteria of individuation, but 
its status as \i{whole} depends on a surrounding 
environment.  We tend to experience qquares on a 
chessboard, or stripes on a flag, or say a solid-color 
placemat, are \i{wholes} because they are \i{parts} of 
a larger totality, against which their individuation can be 
figured.  So, cognitively, parthood and wholeness 
can be interdependent; there ar ecases where something's 
cognitive status as \i{whole} relies on its 
simultaneous status as \i{part}.  Here we do 
not experience comportments to whole-aspects or
part-aspects as in tension; instead, they are 
mutually founding.
}
\p{Jointly, then, \i{individual coherence} and 
\i{internal structuration} present a kind of two-valued 
criteriology modulating appraisals of 
wholeness and parthood.  There are putative 
wholes that seem to slide toward triviality 
because there are few senses in which 
their integral unity is relevant.  Consider the 
British Commonwealth, which nomimally aggregates 
most former UK colonis but would 
rarely be classified as a geopolitical entity 
analogous to the Unitd Nations 
or the European Union, much less a sovereign nation.  
These wholes effectively have 
significant \i{internal structuration} but little 
\i{individual coherence}.  
Meanwhile, there are wholes which seem 
essentially insignificant \mdash{} at 
ast outside contxts where they are 
parts in turn \mdash{} because they have 
\i{individual coherence} but little 
\i{internal structration}.
}
\p{The wholes which are important or meaningful, which 
tend to evolve toward distinct concepts and 
schematic conceptualization \mdash{} 
entrenched in conceptual schema, e.g. via 
natural language lexicons \mdash{} tend to lie 
at the intersection of high apportions 
of both \i{internal structration} and  
\i{individual coherence}.  If we imagine 
these on a two-dimensional \q{scale}, 
say, an \q{IS/IC} scale, cognitively 
significant wholes would tend to be 
situated toward the upper-left of 
region.  
}
\p{An IS/IC scale is at best a suggestive idea, a philosophical 
trope; I don't actually think we should try to \q{quantify} 
thse dimensions.  But as an image I think the 
notion of intersecting \q{IS} and \q{IC} can serve as an 
intuition not only for analyzing cognitive 
frames \mdash{} our perceptual/enactive comportment in 
the momen \mdash{} hut also our conceptual 
systems that condition cognitive dispositions.  Mereological 
models can have noteworthy structural significance 
