...
\p{In short, a \q{voxel} model has rlativly limited usefulness even as 
...
may not be dsired in a mereological thory.  
...
topological deformations of $y$ yielding a $y'$s 
that, without some extra criteria, will also be in $x$.  
We can of courdr stipulate that only some submanifolds 
...
axiomatic mreology invoked as part of a philosophical 
...
restrictions (e.g. distincguishing 
...
accpunt of the relation between 4the 
...
the formal statemeent of the mereological 
...
characterize cpncepts, which can then 
...
\q{addition} between concpts (a variation is to 
consider examples with \i{common} features of 
two concepts: so a potential 
milk-plus-almond-milk concpt would have as examples any 
liquid exhibiting the features milk and 
almond milk have in common).
}
\p{Formal Concept Analysis may provide an interesting systmatization 
of discriminitive or evolutionary factors shaping 
how humans intuit and modify concpts.  There are other 
...
These various theores can be combined; for example, 
...
systematic translation of syrface linguistic structure 
...
where (\ref{itm:sangalong}) is most likely to be epressed 
...
we would make that judgment: there is norhing 
...
more tightly than \i{sing along}.  The situationsl 
model most appropriate for \i{perform} leans toward 
narrow scope, while the situationsl 
...
live in th New York metro area.  In the case of 
...
is \i{most}, not \i{all}; it does jot fit our 
...
this fact is reiterate multiple times by the repeated 
occurance of some particular student expression unhappiness.  
The verbiage used to describe that unhappiness, also 
...
(\ref{itm:borough})-(\ref{itm:tipping}) have a logical 
...
introduce both a global and a local space such that 
...
logical representation which captures at last some 
of the intended meaning of the sentencs.
}
...
different formations taking the palce of 
...
thesher number of students complaining \i{caused} something 
...
specificity of word-senses themselves.
}
...
among physical phenomena; a fature/example statistic 
as a Formal Concept space; a prototyped and dimensionalized 
feature-space as a Conceptual Space theory.  
...
cognitive or scientifuically-described systems 
