\subsection{Multiscientific Objects}
...
archaeological object, say, while the statue 
now missing the piece \i{is} an 
archaeological whole.  Similarly, $Me$ after 
...
$Triv$ is certainly not a \i{psychological} object 
as $Me$ is; I don't feel sensations from 
the snipped-nail part, or see it as myself, 
and so forth.
}
...
instantiate biological properties (like, even 
the snipped-nail-part is composed of cells) 
and the statue-plus-piece instantiates minerological 
properties (even the piece is composed of clay).  
...
(which is presumably a precondition of objects lying 
in the circle of our life world and 
...
typed quantification: when assrting 
properties about an object, we can identify 
the register (or one of the registers) where the  
...
such that...}.  Let's say that for a register \Rcal{} 
we can have \Rcal{}-quantifiers that can iterate over 
things which have an \Rcal{}-aspect.  Thus an archaeological 
object is one that can be in the domain 
of an archaeological-quantifier.
}
\p{My term \q{\Rcal{}-quantifier} is at least distantly related 
...
of an orchestra.  Wiegand initially 
defines \q{$R$-structure} in formal terms 
but he soon thereafter, referencing Husserl, 
implies that contexture can be read 
as a \i{phenomenological} context, the 
cognitive/enactive considerations 
that dispose me to attend to a whole 
within its surroundings.  Referring back to my 
discussion in Section 2 (including 
my reference to an earlier essay of Wiegand's), phenomenological 
...
Physical objects are unified in terms of 
...
idea that \q{$R$-structured} wholes are
...
quantifers, or of combined quantifers.  If \Rcal{} and 
\Rcal{}' are registers, then an \Rcal{}/\Rcal{}'-quantifier iterates 
over things that have a facet as \Rcal{} and also a facet as 
\Rcal{}'.  Inter-register dependence means that some 
quantifiers are necessarily mixed: an 
archaeoloical-quantifer is necessarily actually 
an \Rcal{}/\Rcal{}'-quantifier where \Rcal{} is archaeology and 
\Rcal{}' is physicality.
}
\p{I believe that a notion of \Rcal{}-quantifiers is a 
natural correlate of what I am calling 
\q{multiscientific mereology}.  Indeed, perhaps 
we should entrench the notion as 
\q{\Rcal{}-quantifier mereology}.  On this account, 
...
an embedded \Rcal{}-quantifier: that a statue 
...
\Rcal{} from which $x$ is drawn via an 
implicit \Rcal{}-quantifier (and the \Rcal{} may actually be a 
mix of registers).
}
\p{I'll say that an \Rcal{}-quantifier mereology is one where 
\i{any} whole that can be part of \xhppy{} formulae 
has an implicit \Rcal{}-quantifier, and therefore has an 
\Rcal{}-facet for some \Rcal{} (or a mixture of \Rcal{}-facets).  
On a multiscientific account, most or all 
cognitively significant \Rcal{}-quantifiers actually involve a 
...
different \Rcal{}s (and the granularity increases because each 
mixture of \Rcal{}s gives rise to a different sort of quantifier.
}
\subsubsection{\Rcal{}-Quantification and Non-Antisymmerty}
\p{Adding \Rcal{}-quantification casts a different perspective on 
...
\q{mixed quantifers}: the \Rcal{}-quantifier 
...
and minerological facets, because it falls under the 
...
\Rcal{}-quantifier mereology could block that kind of formula 
because there is no apparent \Rcal{}-quantifier 
...
different \Rcal{}-quantification domains, because 
...
However, the \q{rules} for such \q{cross-\Rcal{}} 
domain-switching may be relatively restricted; we are  
not forced to theorize mereological systems 
where the component \Rcal{}s in a mixed-register 
domain can be generically factored out to their 
own quantification-domains. 
}
\p{Moreover, this restriction captures why \Rcal{}-quantification 
produces a more regulated mereological structure, 
because ther are limited \q{parthood} relations 
between \Rcal{}s in a mixture themselves.  In an 
\Rcal{}/\Rcal{}'-quantification such as archaeological-minerological, 
it might be said that the specified domains 
(archaeological and minerological) are \q{parts} of 
the mixed \Rcal{} which governs the quantification 
...
related properties.  Analogously, even if 
we accept the language of an \Rcal{}-quantifier as 
...
\Rcal{}-quantification, this would require a distinct 
domain of quantification which I have not yet pondered 
\mdash{} I'll call it \Xcal{}.  Of course, the actual 
domain for the sculpture combines (at least) 
archaeological and minerological, but 
this mixture is not \Xcal{}, because we want the various 
\i{facets} to be \i{part of} the \Xcal{}-object 
in the internal vocabulary of a mereology.  I have 
allowed that \Rcal{}-mixtures are in some sense 
...
under one quantifier).  Someone might find 
...
domain abstracting from finer \Rcal{}-domains.  
So quantifying over this general 
domain \mdash{} say, \Xcal{}-quantification \mdash{} 
models a different semantic pattern 
than \Rcal{}-quantification.  Whereas \Rcal{}-quantification 
reflects expressions like \i{there is a sculpture 
such that...}, \Xcal{}-quantification corresponds 
to the more symbolic logical formulation like 
\i{there exists \underline{x} such that...}.
}
\p{It might seem as if \Xcal{}-quantification can be logically 
converted to \Rcal{}-quantification by, in effect, 
treating the \Rcal{}-quantifier as a predicate under the 
scope of the (purported) \Xcal{}-quantifier.  So from 
...
That is, \i{\underline{x}} is an \Xcal{}-object, under the 
domain of a quantifier without any 
ontological structure or specificity.  
If any \Xcal{}-object can be a whole, then we have mereological 
relations outside the regulations of \Rcal{}-quantification.  
For instance, we have a stated theory of why $Triv$ 
is trivial; because there is no \Rcal{}-quantifier, for any 
\Rcal{}, which covers $Triv$ (or at last there is 
no multiscientific mixture of \Rcal{}s that 
would render $Triv$ a \q{multiscientific} whole).  
However, it seems reasonable 
to allow that $Triv$ is an \Xcal{}-object, because by 
definition \Xcal{}-quantification does not impose 
...
about \Xcal{}-quantification.  If there is only 
\Rcal{}-quantification, then there are only nontrivial 
...
on our beliefs about quantification.  If one is 
already committed to the idea that \Xcal{}-quantification is 
possible \mdash{} or that philosophers at least have to give 
...
unrestricted or \Xcal{}-quantification.  On the one hand, 
formal logic certainly accepts leaving quantification open-ended.  
It's not clear what we should read into that philosophically 
\mdash{} after all, logic is a study of abstract domains, 
rather than an empicial universe of statues and toes 
and such.\footnote{Also, even in mathematics we have type-theoreric 
...
accepted even in logic and mathematics.  
Granted, though, type-restricted quantification 
is not the predominant logical construct.
}  
Another philosophical case for \Xcal{}-quantification 
is that recognizing only \Rcal{}-quantification 
\i{without abstracting to} an unrestricted 
\q{\Xcal{}-domain} would seem to leave the \Rcal{}-domains 
bearing a lot of weight.  It is one thing 
to quantify over a set of objects bearing biological 
properties; it is another to establish a rule 
of quantification presupposing that there is some 
essential register in existence containing 
all and only biological objects.
}
\p{We moderns presumably reject thoughts of an \i{elan vital} 
...
properties, which \i{makes} them biological or minerological 
objects.  But that sounds as if we envision a 
baseline of \Xcal{}-quantification wherein biological 
and minerological domains, and so forth, 
are somehow \q{emergent} or higher-scale properties.  
Foreclosing \Xcal{}-quantification may appear to leave 
us with inexplicable \Rcal{}-domains, which have 
to be just accepted as metaphysical primitives, 
as if they are primordial \q{elans}.  
I'll call this the \q{\i{elan}} problem. 
}
\p{I claim, though, that from the 
arguing-against-unrestricted-quantification 
...
talk into a \q{prestige dialect}, an ontologically 
...
\p{But such an inscription of \Xcal{}-quantification 
has its own blind-spots.  The objects 
of the subatomic domain are not metaphysical 
essences outside the play of science; they are posits of a 
...
and dispersive existence.  When we knock on wood 
...
of atoms and molecules \mdash{} or, say, 
...
the conceptual merger of these ideas creates the notion 
...
such perpetual exchange of photons prohibits 
any direct enumeration 
...
Presumably sufficiently scattered sums are trivial, 
but we can't assume any physically connected mass of 
ur-particles is non-trivial (even if we accept the approximation 
that solid objects are \i{really} solid).  
A glass atop a table forms a connected but, we'd say, 
...
A bucket of sand or glass of water have states where physical 
...
\p{So, while it may be appealing to adopt a discource 
of \q{ur-particles} to encapsulate our atomistic and 
...
philosophical improvement over \Rcal{}-quantification, even if 
this seems to lead us back to the question of how 
to think through \Rcal{}-quantifiers without 
conjuring a realm of \Rcal{}-elans.
}
...
\Rcal{}-objects to sustain as non-trivial wholes is 
complex, and depends on laws and dynamics 
investigated by \Rcal{}-sciences themselves.  We do not have a 
succinct logical statement of what makes \Rcal{}-quantified 
mereological wholes non-trivial.  This does not mean we 
fall back on spooky \i{elans}; but rather that, for a 
given \Rcal{}-quantifier, we can form (with sufficient scientific 
input) a \i{theory} of non-triviality in the 
\Rcal{}-domain.  Biologists, for example, do not rely 
...
archaeologists the demarcation of archaeological objects, and so forth.
}
...
with an inventory of theories drawn from 
multiple sciences, of what makes a biological object 
biological, etc.  While a prima facie 
claim that sums with no \Rcal{}-quantifier just are trivial, 
a rstriction that seems question-begging, 
seems at best philosophically incomplete, we 
can complete the picture by introducing 
constitutive theories developed by sciences 
to identify their ontic extent.  For any \Rcal{}-domain, 
we can assume that there is a scientific 
theory of \Rcal{}-ness; of the conditions where 
objects are \Rcal{}-objects.  These theories may be provisional 
an \i{a posteriori}, but they can complement 
an underlying mereological theory: 
a whole is non-trivial if it lies 
in the domain of an \Rcal{}-quantifier, and we can philosophically 
defer to a suite of empirical theories delimiting 
\Rcal{}-objects for different \Rcal{}'s; the corresponding 
\q{\Rcal{}-theories} may lie outside philosophy proper, but 
they are not intellectually vacant \i{elans}.   
}
\subsubsection{\Rcal{}-Quantification and Cognitive Frames}
\p{My analysis of \Rcal{}-quantification has been driven by a desire 
...
objects, the metaphysical problems of triviality may 
never arise.  Non-trivial wholes 
in \i{cognitive} mereology are non-trivial 
because we have innate perceptual tendencies and 
learned conceptualizations that dispose us to 
...
define mereological relations as extramental features 
compelling or at least influencing those cognitive 
manifestations.  Without the ambient 
...
meaningful, naturally integral, non-trivial wholes 
are modeled by \xhppy{}-like formulae.  
}
\p{In sbort, I propose \Rcal{}-quantification not as the starting point 
of a mereological theory but in conjunction with \i{cognitive} 
mereology: we want to pass beyond a theory 
wherein mereological relations are \i{only} cognitive 
without thereby ending up in vague ur-particle-like talk wherein  
everything extra-mental is a mereological sum of 
...
(i.e., extramental) reality is ontologically 
...
mereological universe ventures outside the mind it 
...
trivial \q{\Xcal{}-quantified} wholes.
}
\p{The problem in this intellectual trajectory is 
the originary intuition that extramentality automatically takes 
into a territory of ur-particles, as if we have 
to philosophically construct some minimal quantum 
of the extramental world.  We don't need to have this intuition.  
We can instead say that extramental 
objects are \Rcal{}-quantified for various \Rcal{}s: outside 
...
pathwork the \Rcal{}-patchwork.
}
\p{I'd like to argue that the \Rcal{}-patchwork functions alot 
like cognitive frames.  The \Rcal{}-patchwork contributes 
extramentally as cognitive frames contribute mentally: 
...
\p{Perhaps this resonance is not surpising: science is shaped by 
...
discipline guides us toward empirical models that 
can be tested apart from human beliefs and 
prejudices, but the structural outline of the sciencs and 
their interrelationships still draws on 
the contrasts between different realms of 
...
are intellectually intuitive 
but end up being empirically misleading.
Extramental reality is not essentially 
\i{non-cognitive}, which is not to say that our 
cognitive actions \q{create} extramental reality, but 
...
outside the cognitive realm does not mean 
an unrestricted quantification with no intellectual 
structure, or within a quantification domain 
that never traces back at some level 
to cognitive attitudes.  \q{Extramental} implies a 
discipline correcting for cognitive prejudice, but 
...
\EnglischeLinie{}
\p{The upshot of my argument in this section was 
that the realm of mereological theory should 
...
and in the second case from an \q{\Rcal{}-patchwork}: 
...
cognitive frames and the \q{\Rcal{}-patchwork}.
}
\p{Whether from the perspective of cognitive frames or the 
\q{extramental} perspective of the \Rcal{}-patchwork, 
...
I'll expand on this hypothesis in the next section.
}
