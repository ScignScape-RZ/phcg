...
reason through some such interprtive frame, so the speaker's role is
...
interprtive \q{script}, is a proposal to the effect that when trying
...
\end{sentenceList}
Likewise in (\label{ref:teamflew}), only the athletes are referenced via
\i{they played}; but presumably many other people (trainers, coaches, staff)
are encompassd by \q{the team flew}.  And in (\label{ref:theflies})
